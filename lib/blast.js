...
class blast {
    ...
    blastDbAlias(option, cb) {
        ...
        _.merge(op, option);
         op.dblist=_.map(option.dblist,(x)=>path.join(op.directory,x));
        var makeCommand = ' blastdb_aliastool -dblist "' + op.dblist.join(" ") + '" -dbtype ' + op.dbtype + ' -out ' + path.join(op.directory,op.out) + ' -title ' + option.title;
